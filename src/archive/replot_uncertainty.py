...
# INPUTS (edit these)
# -----------------------------
sigma_csv = Path(r"C:\VOW\res\south\exp_20\9_leaveout_uncert\leaveout_sigma_map.csv")
coords_csv = Path(r"C:\VOW\res\south\exp_20\3_sections\cpt_coords_with_distances.csv")
out_png = Path(
    r"C:\VOW\res\south\exp_20\9_leaveout_uncert\leaveout_sigma_map_FIXED.png"
)
...
if xmin_m is None:
    xmin_m = float(coords["cum_along_m"].min())
if xmax_m is None:
    xmax_m = float(coords["cum_along_m"].max())
...
plt.title("Leave-out uncertainty (Std Dev)")
