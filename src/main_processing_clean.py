...
REGION = "north"
EXP_NAME = "exp_1"
DESCRIPTION = "Complete VOW SchemaGAN pipeline - coordinates extraction to mosaic generation"
...
def run_cpt_data_processing(cpt_folder: Path, output_folder: Path, compression_method: str = "mean"):
    """Wrapper for extract_data.py functionality."""
    from extract_data import (
        process_cpts, equalize_top, equalize_depth, 
        compress_to_32px, save_cpt_to_csv
    )
...
def run_section_creation(coords_csv: Path, cpt_csv: Path, output_folder: Path, 
                        n_cols: int, n_rows: int, cpts_per_section: int, 
                        overlap: int, left_pad: float, right_pad: float,
                        dir_from: str, dir_to: str):
    """Wrapper for create_schGAN_input_file.py functionality."""
    from create_schGAN_input_file import process_sections, write_manifest, validate_input_files
    import pandas as pd
    ...
    manifest = process_sections(
        coords_df=coords_df, cpt_df=cpt_df, out_dir=output_folder,
        n_cols=n_cols, n_rows=n_rows, per=cpts_per_section, overlap=overlap,
        left_pad_frac=left_pad, right_pad_frac=right_pad,
        from_where=dir_from, to_where=dir_to,
    )
...
def run_schema_generation(sections_folder: Path, gan_images_folder: Path, 
                         model_path: Path, y_top_m: float, y_bottom_m: float):
    ...
    create_schema.COORDS_WITH_DIST_CSV = sections_folder / "cpt_coords_with_distances.csv"
    ...
    for i, section_file in enumerate(section_files, 1):
        try:
            ...
            success_count += 1
            logger.info(f"[{i:03d}/{len(section_files)}] Generated: {csv_out.name} & {png_out.name}")
        except Exception as e:
            fail_count += 1
            logger.error(f"[{i:03d}/{len(section_files)}] Failed on {section_file.name}: {e}")
    
    logger.info(f"Schema generation complete. Success: {success_count}, Failed: {fail_count}")
    return success_count, fail_count


def run_mosaic_creation(sections_folder: Path, gan_images_folder: Path, 
                       mosaic_folder: Path, y_top_m: float, y_bottom_m: float):
    ...
    create_mosaic.COORDS_WITH_DIST_CSV = sections_folder / "cpt_coords_with_distances.csv"
...
def main():
    ...
    try:
        manifest = run_section_creation(
            coords_csv, compressed_csv, folders["3_sections"],
            N_COLS, N_ROWS, CPTS_PER_SECTION, OVERLAP_CPTS,
            LEFT_PAD_FRACTION, RIGHT_PAD_FRACTION, DIR_FROM, DIR_TO
        )
    except Exception as e:
    ...
    if not SCHGAN_MODEL_PATH.exists():
        ...
        try:
            run_schema_generation(
                folders["3_sections"], folders["4_gan_images"], 
                SCHGAN_MODEL_PATH, Y_TOP_M, Y_BOTTOM_M
            )
        except Exception as e:
    ...
    try:
        run_mosaic_creation(
            folders["3_sections"], folders["4_gan_images"], 
            folders["5_mosaic"], Y_TOP_M, Y_BOTTOM_M
        )
    except Exception as e:
    ...
    # Summary statistics
    logger.info("\nPipeline Summary:")
    logger.info(f"  - Sections created: {len(manifest) if 'manifest' in locals() else 'N/A'}")
